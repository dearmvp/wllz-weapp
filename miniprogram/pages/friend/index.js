...
Page({
    ...
    async loadData() {
        ...
        for (let index = 0; index < 26; index++) {
            ...
            alphaTemp.push(String.fromCharCode((65 + index)))
        }
        let noletter = {
            alpha: '#',
            subItems: []
        ...
        if (res.success) {
            for (const item of res.data) {
                ...
                if (alphaTemp.indexOf(firstLetter.toUpperCase()) < 0) {
     ...
     */
    onShow() {
        let friend_load_flag = wx.getStorageSync("friend_load_flag")
        if (friend_load_flag) {
            this.loadData()
            wx.removeStorageSync("friend_load_flag")
        }
    },
